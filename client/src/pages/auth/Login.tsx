...
import { scaleUp, loginArgs } from "../../utils";
import { useMainContext } from "../../context/MainContext";
import { EyeButton, FomikError, FormButton, FormHead, FormikField, RouteLink } from "../../components";
          ...
          >
            {(formik) => (
              <Form className="flexColItemCenter p-3.5 gap-y-1.5">
                {_.map(loginArgs, ({ name, placeholder }, id) => (
                  <div key={id} className="w-full">
                    <div className="relative ">
                      <FormikField visible={visible} name={name} placeholder={placeholder} />
                      {id === 1 && <EyeButton setVisible={setVisible} visible={visible} />}
                    </div>
                    <FomikError name={name} />
                  </div>
                ))}
                <FormButton isSubmitting={formik.isSubmitting} name="Log In" />
                <RouteLink name="Signup" path="/auth/signup" />
              </Form>
            )}
          </Formik>
        </motion.div>
